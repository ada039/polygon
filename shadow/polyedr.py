...
class Facet:
    ...
    def is_good_facet(self):
        for vertex in self.vertexes:
            if not vertex.is_good_point():
                return False
        if not self.center().is_good_point():
            return False
        return True
...
class Polyedr:
    ...
    def __init__(self, file):
        ...
        self.origvertexes, self.origedges, self.origfacets = [], [], []
        ...
        with open(file) as f:
            for i, line in enumerate(f):
                if i == 0:
                    ...
                    origvertexes = list(
                        self.origvertexes[int(n) - 1] for n in buf)
                    # задание рёбер грани
                    for n in range(size):
                        self.edges.append(Edge(vertexes[n - 1], vertexes[n]))
                        self.origedges.append(
                            Edge(origvertexes[n - 1], origvertexes[n]))
                    # задание самой грани
                    self.facets.append(Facet(vertexes))
                    self.origfacets.append(Facet(origvertexes))

            self.area = self.good_area_sum()

    def good_area_sum(self):
        total_area = 0.0
        for i in range(len(self.origfacets)):
            if self.facets[i].is_good_facet():
