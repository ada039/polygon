...
class R3:
    ...
    # Хорошая ли точка
    def is_good_point(self):
        square4 = (abs(self.x) < 1) and (abs(self.y) < 1)
        square1 = (abs(self.x) <= 0.5) and (abs(self.y) <= 0.5)
        return square4 and not square1

    # Площадь треугольника
    @staticmethod
    def triangle_area(p1, p2, p3):
        vec = (p2 - p1).cross(p3 - p1)
        len = sqrt(vec.dot(vec))
        return len/2
